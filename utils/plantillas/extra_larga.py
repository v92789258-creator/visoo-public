"""
Plantilla de boleta extra larga (versión profesional) para impresoras térmicas.
Comparte la lógica avanzada de la plantilla pequeña pero con configuración específica.
"""
...
class PlantillaExtraLarga(PlantillaBase):
    """Genera boletas 'extra largas' (ahora con altura dinámica optimizada) con QR y diseño profesional."""
    
    CONFIGURACION = {
        'ancho': 80,
        # 'alto' ya no es fijo, se calcula dinámicamente
        'margen': 3,
        'font_titulo': 10,
        'font_subtitulo': 9,
        'font_normal': 8,
    ...
    def generar(self, datos_boleta, ruta_salida=None):
        """Genera la boleta con altura exacta (Simulación previa)."""
        ...
        print(f"\n{'='*70}")
        print(f"[BOLETA EXTRA LARGA] INICIANDO GENERACIÓN OPTIMIZADA...")
        ...
        print(f"[EXITO] Boleta profesional generada: {os.path.basename(result)}")
        print(f"{'='*70}\n")
    ...
    def _dibujar_encabezado(self, pdf, datos, config, y, ancho, tiene_logo, ruta_logo, config_sunat=None):
        ...
        if tiene_logo:
            try:
                ...
                y += w_logo + 2
                # LOGO PRESENTE: NO dibujamos nombre_empresa
                nombre_dibujado = True 
            except:
                y += 5
        
        if not nombre_dibujado:
            # Solo si NO hubo logo, dibujamos el nombre
            pdf.set_font('Helvetica', 'B', 11)
            pdf.set_xy(config['margen'], y)
            pdf.multi_cell(ancho, 5, self._limpiar_texto(nombre_empresa), 0, 'C')
            y = pdf.get_y() + 1
        
        # Dirección y Contacto (Siempre se muestran)
    ...
    def _dibujar_info_documento(self, pdf, datos, config, y, ancho):
        """Dibuja número de boleta, fecha, cliente y vendedor."""
        ...
        # Vendedor (Siempre visible en Extra Larga si existe)
    ...
    def _dibujar_productos(self, pdf, datos, config, y, ancho):
        """Dibuja la lista de productos con detalles extra (P.U.)."""
        # Encabezados
        pdf.set_font('Helvetica', 'B', 7)
        pdf.set_xy(config['margen'], y)
        
        col_cant = 12  # Un poco más ancho para "Cant x P.U"
        col_tot = 15
        ...
        for p in self._normalizar_productos(datos.get('productos', [])):
            ...
            precio_unit = p.get('precio', 0)
            total = p.get('total', 0)
            
            y_inicio = y
            
            # Cantidad y Precio Unitario debajo
            pdf.set_xy(config['margen'], y)
            pdf.cell(col_cant, 4, f"{cant}", 0, 0, 'C')
            
            # Detalle P.U. en fuente pequeña debajo de la cantidad o descripción
            # En este diseño, ponemos el P.U. en la descripción para mayor claridad
            texto_desc = f"{nombre}\n(P.U: S/{precio_unit:.2f})"
            
            # Imprimir Total
            pdf.set_xy(config['margen'] + col_cant + col_desc, y)
            pdf.cell(col_tot, 4, f"{total:.2f}", 0, 0, 'R')
            
            # Imprimir descripción con P.U.
            pdf.set_xy(config['margen'] + col_cant, y)
            pdf.multi_cell(col_desc, 4, texto_desc, 0, 'L')
            
            y = max(pdf.get_y(), y_inicio + 8) # Asegurar espacio para las 2 líneas
    ...
    def _dibujar_totales(self, pdf, datos, config, y, ancho):
        ...
        # Mostrar pago parcial si existe (ANTES del total)
